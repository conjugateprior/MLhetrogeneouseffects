...
data_uganda_train <- data_uganda_na[rv < 0.75, ]
data_uganda_test <- data_uganda_na[rv >= 0.75, ]
...
#data_uganda_train <- cbind(data_uganda_train, Y_outcome)
...
C_vars <- as.matrix(data_uganda_train$vill_id, data_uganda_train$dist_nearclub, data_uganda_train$club_frequently, data_uganda_train$club_sometimes )
str(C_vars)
...
preds_IGA <- predict(
  ...
  newdata = model.matrix(~ children, data = data_uganda_test), 
