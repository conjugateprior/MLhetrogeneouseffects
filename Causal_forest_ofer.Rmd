...
X.test.basic <- model.matrix(lm(foll_index_income_gen_act ~ 
                                  children + children_na +
                                  branchno + branchno_na +
                                  age + age_na +
                                  rural + rural_na +
                                  income + income_na + 
...
```{r}
#standard error predictions
standard.error = sqrt(pred.cf.basic.imp$variance.estimates)
...
plot(tree <- get_tree(cf.basic.imp, 3))
tree$nodes
